...
public:
    ...
    */
	...
	/// Bind a class property (variable accessed via a setter and getter) DeXmas: Overload for pointers
    template<class V>
    Class& Prop(const SQChar* name, V (C::*getMethod)() const, void (C::*setMethod)(const V)) {
        if(getMethod != NULL) {
            // Add the getter
...
protected:
	...
	static SQInteger ClassGetClass(HSQUIRRELVM vm)
	{
    ...
    void InitClass() {
		...
		// add getclass (apparently not provided by default)
        sq_pushstring(vm, _SC("getclass"), -1);
		sq_newclosure(vm, &Class::ClassGetClass, 0);
        sq_newslot(vm, -3, false);
...
protected:
    void InitDerivedClass(HSQUIRRELVM vm) {
		...
		// add getclass (apparently not provided by default)
        sq_pushstring(vm, _SC("getclass"), -1);
		sq_newclosure(vm, &Class<C, A>::ClassGetClass, 0);
        sq_newslot(vm, -3, false);
